...
class InterfaceVerify(object):
    ...
    @staticmethod
    @make_decorator
    def verify_update_interface_detail(interface_id, interface_name, interface_desc, interface_index, run_time, retry,
                                       user_id, is_deleted):
        """修改任务流详情"""
        if not interface_id:
            abort(400, **make_result(status=400, msg='任务流id不存在'))
        ...
        return Response(interface_id=interface_id, interface_name=interface_name, interface_desc=interface_desc,
                        interface_index=interface_index, run_time=run_time, retry=retry, user_id=user_id,
