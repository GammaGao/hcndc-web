...
class ExecuteVerify(object):
    ...
    @staticmethod
    @make_decorator
    def verify_get_execute_flow(interface_id, interface_index, run_status, run_date, page, limit):
        """获取任务流日志"""
        # 任务流目录
        ...
        return Response(interface_id=interface_id, interface_index=interface_index, run_status=run_status,
                        run_date=run_date, page=page, limit=limit)

    @staticmethod
    @make_decorator
    def verify_get_execute_flow_history(dispatch_id, run_date, run_status, page, limit):
        """获取任务流历史日志"""
        # 调度ID
        if not dispatch_id:
        ...
        return Response(dispatch_id=dispatch_id, run_date=run_date, run_status=run_status, page=page, limit=limit)

    @staticmethod
    @make_decorator
    def verify_get_execute_job_log_list(job_id, start_time, end_time, run_status, page, limit):
        """获取手动执行任务日志"""
        # 执行时间
    ...
    @staticmethod
    @make_decorator
    def verify_get_execute_job_history(job_id, exec_type, start_time, end_time, run_status, page, limit):
        """获取任务历史日志列表"""
        # 执行时间
        if start_time and end_time and start_time >= end_time:
            abort(400, **make_result(status=400, msg='执行开始时间大于创建结束时间'))
        ...
        return Response(job_id=job_id, exec_type=exec_type, start_time=start_time, end_time=end_time,
                        run_status=run_status, page=page, limit=limit)
