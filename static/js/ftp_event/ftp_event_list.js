    ...
    Controller.prototype = {
        ...
        table_data_load: function (data) {
            ...
            let toolbar_div = [
                ...
                '<div class="layui-inline" lay-event="update" title="修改"><i class="layui-icon layui-icon-edit"></i></div>',
            ...
            layui.use('table', function () {
                ...
                table.render({
                    elem: "#file-event-list",
                    // height: 40,
                    page: true,
                    toolbar: toolbar_div,
                    limits: [10, 20, 30, 40, 50],
                    title: '文件事件',
                    url: BASE.uri.ftp_event.list_api,
                    where: data,
                    cols: [[{
                        type: 'checkbox'
                    }, {
        ...
        toolbar_data_event: function () {
            ...
            layui.use('table', function () {
                let table = layui.table;
                table.on('toolbar(file-event-list)', function (obj) {
                    // 工具栏事件监听
                    ...
                    switch (obj.event) {
                        ...
                        case 'add':
                            layer.open({
                                ...
                                maxmin: true,
                                area: ['60%', '80%'],
                                content: BASE.uri.ftp_event.add,
                                end: function (index) {
                        ...
                        case 'update':
                            if (check_data.length === 0) {
                                ...
                                layer.open({
                                    ...
                                    maxmin: true,
                                    area: ['60%', '80%'],
                                    content: BASE.uri.ftp_event.update + check_data[0].ftp_event_id + '/',
                                    end: function (index) {
                                        $(".layui-laypage-btn").click();
                                        layer.close(index);
                                    }
                        ...
                        case 'run':
                            ...
                            if (delete_status.length > 0) {
                                ...
                                let run_id_arr = [];
                                check_data.forEach(item => run_id_arr.push(item.ftp_event_id));
                                layer.open({
                                    type: 2,
                                    anim: 5,
                                    title: '立即执行',
                                    maxmin: true,
                                    area: ['60%', '80%'],
                                    content: BASE.uri.ftp_event.run + '?ftp_event_id=' + run_id_arr.join(',')
                                });
                            }
                            break;
                        // 暂停
                        case 'pause':
                            ...
                            if (pause_status.length > 0) {
                                ...
                                let pause_id_arr = [];
                                check_data.forEach(item => pause_id_arr.push(item.ftp_event_id));
                                $.ajax({
                                    url: BASE.uri.ftp_event.action_api,
                                    contentType: "application/json; charset=utf-8",
                                    type: 'patch',
                                    data: JSON.stringify({ftp_event_id: pause_id_arr, 'action': 1}),
                                    success: function (result) {
                        ...
                        case 'resume':
                            ...
                            if (resume_status.length > 0) {
                                ...
                                let resume_id_arr = [];
                                check_data.forEach(item => resume_id_arr.push(item.ftp_event_id));
                                $.ajax({
                                    url: BASE.uri.ftp_event.action_api,
                                    contentType: "application/json; charset=utf-8",
                                    type: 'patch',
                                    data: JSON.stringify({ftp_event_id: resume_id_arr, 'action': 2}),
                                    success: function (result) {
                                        if (result.status === 200) {
                                            layer.open({
                        ...
                        // 删除
                        case 'delete':
                            let ftp_event_id_arr = [];
                            check_data.forEach(item => ftp_event_id_arr.push(item.ftp_event_id));
                            layer.confirm('确定删除?', function (index) {
                                // 关闭弹窗
                                layer.close(index);
                                $.ajax({
                                    url: BASE.uri.ftp_event.action_api,
                                    data: JSON.stringify({ftp_event_id: ftp_event_id_arr}),
                                    contentType: "application/json; charset=utf-8",
                                    type: 'delete',
                                    success: function (result) {
                                        if (result.status === 200) {
        ...
        table_data_event: function () {
            layui.use('table', function () {
                let table = layui.table;
                table.on('tool(file-event-list)', function (obj) {
                    let data = obj.data;
                    let event = obj.event;
                    switch (event) {
                        // 检测目录
                        ...
                        case 'run':
                            layer.open({
                                ...
                                maxmin: true,
                                area: ['60%', '80%'],
                                content: BASE.uri.ftp_event.run + '?ftp_event_id=' + data.ftp_event_id
                            });
                            break;
                        // 暂停
                        case 'pause':
                            $.ajax({
                                ...
                                contentType: "application/json; charset=utf-8",
                                type: 'patch',
                                data: JSON.stringify({ftp_event_id: [data.ftp_event_id], 'action': 1}),
                                success: function (result) {
                                    if (result.status === 200) {
                                        layer.open({
                                            id: 'dispatch_pause_succeed',
                                            title: '暂停调度成功',
                                            content: '暂停调度id: ' + data.ftp_event_id + '成功',
                                            yes: function (index) {
                                                // 刷新页面
                                                $(".layui-laypage-btn").click();
                                                layer.close(index);
                        ...
                        case 'resume':
                            $.ajax({
                                ...
                                contentType: "application/json; charset=utf-8",
                                type: 'patch',
                                data: JSON.stringify({ftp_event_id: [data.ftp_event_id], 'action': 2}),
                                success: function (result) {
                                    if (result.status === 200) {
                                        layer.open({
                                            id: 'dispatch_resume_succeed',
                                            title: '恢复调度成功',
                                            content: '恢复调度id: ' + data.ftp_event_id + '成功',
                                            yes: function (index) {
                                                // 刷新页面
                                                $(".layui-laypage-btn").click();
                                                layer.close(index);
                        ...
                        case 'update':
                            layer.open({
                                ...
                                maxmin: true,
                                area: ['60%', '80%'],
                                content: BASE.uri.ftp_event.update + data.ftp_event_id + '/',
                                end: function (index) {
                                    $(".layui-laypage-btn").click();
                                    layer.close(index);
                                }
                        ...
                        case 'delete':
                            layer.confirm('确定删除?', function (index) {
                                ...
                                $.ajax({
                                    url: BASE.uri.ftp_event.action_api,
                                    data: JSON.stringify({ftp_event_id: [data.ftp_event_id]}),
                                    contentType: "application/json; charset=utf-8",
                                    type: 'delete',
                                    success: function (result) {
                                        if (result.status === 200) {
